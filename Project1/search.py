    ...
    while not myStack.isEmpty():  
        ...
        if currentState not in visited:
            ...
            for nextState, action, cost in problem.getSuccessors(currentState):
                myStack.push((nextState, path + [action]))
...
def breadthFirstSearch(problem: SearchProblem):
    ...
    while not myQueue.isEmpty():
        ...
        if currentState not in visited:
            visited.add(currentState)
            for nextState, action, cost in problem.getSuccessors(currentState):
                myQueue.push((nextState, path + [action]))
...
def uniformCostSearch(problem: SearchProblem):
    ...
    while not myPriorityQueue.isEmpty():
        ...
        if currentState not in visited:
            visited.add(currentState)
            for nextState, action, cost in problem.getSuccessors(currentState):
                myPriorityQueue.push((nextState, path + [action]), problem.getCostOfActions(path + [action]))
...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic):
    ...
    while not myPriorityQueue.isEmpty():
        ...
        if currentState not in visited:
            visited.add(currentState)
            for nextState, action, cost in problem.getSuccessors(currentState):
                myPriorityQueue.push((nextState, path + [action]), problem.getCostOfActions(path + [action]) + heuristic(nextState, problem))
    return[]
